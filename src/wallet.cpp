...
// MEC: optional setting to unlock wallet for block minting only;
...
// MEC: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // MEC: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // MEC: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    //  post-backup change.
#ifdef WIN32
                    if (!GetBoolArg("-avatar", true)) // MEC: not avatar mode; mec: avatar mode enabled by default to avoid change being sent to hidden address
#else
...
// MEC: create coin stake transaction
...
// MEC: check 'spent' consistency between wallet and txindex
// MEC: fix wallet spent state according to txindex
...
// MEC: disable transaction (only for coinstake)
